...
def data_to_graph(inputDir, outputDir, type, exampleGraph=False):
    ...
    inputFiles_list = list(filter(lambda f: f.endswith('.root'), inputFiles_list))
    os.chdir(files_path)
    ...
    for inputFile in inputFiles_list:
        ...
        for i in range(nEvents):  # Iterate through graphs
            ...
            if exampleGraph and i % 1000 == 0:  # Printing example graphs
                try:
                ...
                plt.savefig(str(i) + 'graph.png', dpi=600)
                plt.clf()
                fig, axs = plt.subplots(2, 1)
                fig.patch.set_visible(False)

                properties = ['$\mathit{\u2113}$', '$\mathit{j}$', '$\mathit{p_T}$', '$\mathit{m}$', '$\mathit{c}$', '$\mathit{CSVv2}$']
                axs[0].axis('off')
                axs[0].axis('tight')
                x = pd.DataFrame(x, columns=properties)
                axs[0].table(cellText=x.values, colLabels=x.columns, loc='center')
                axs[1].axis('off')
                axs[1].axis('tight')
                axs[1].table(edge_features_matrix)
                fig.tight_layout()
                plt.savefig(str(i) + 'features.png', dpi=600)
                plt.clf()
...
top_quarks = 3  # 0 for 2, 1 for 4 events

data_to_graph(input_directory, output_directory, top_quarks, exampleGraph=True)
